...
const ActionCard = ({action}: ActionCardProps) => {
    const{img, title, author, description, address, amountCollected, totalGoal, finalDate} = action
    return(
        <div className="flex flex-col gap-4 rounded-[10px] overflow-hidden shadow-md w-full max-w-[420px] min-w-[320px]">
            ...
            <div className="flex flex-col py-8 px-6">
                ...
                <ProgressBar percent={amountCollected / totalGoal} firstElement={<div className="flex flex-col items-center text-sm"><h3 className="font-bold">R$ {amountCollected} </h3><h4>Arrecadados</h4></div>} secondElement={<div className="flex flex-col items-end text-sm"><h3 className="font-bold"> 7 dias </h3><h4>restantes</h4></div>}/>
            </div>
            <Button iconLeft="fluent:handshake-24-filled" className="py-6 text-xl">Apoiar Ação</Button>
        </div>
    )
}
